...
const useControler = {
  ...
  async listarUsuariosPorUsuarioDiferente(req, res) {
    try {
      const users = await userDao.getUsersDifferent(req.params.id);
      res.json({ message: "Usuários recuperados com sucesso!.", type: "e", users: users });
    } catch (error) {
      res.status(500).json({ error: "Erro ao listar usuários.", type: "e" });
    }
  },
  ...
  async listaUsuarioPorId(req, res) {
    try {
      ...
      res.status(400).json({ error: "Erro ao recuperar usuários: ", type: "e" });
    }
  },
  ...
  async editarUsuario(req, res) {
    try {
      ...
      const updatedUser = new userModel(req.body.userData);
      ...
      if (!userToEdit) {
        ...
        userToEdit.userType = updatedUser.userType;
        userToEdit.role = updatedUser.role;
  ...
  async loginUsuario(req, res) {
    try {
      ...
      res.status(200).json({ message: "Login bem-sucedido", token, type: "s", user: loginUser });
    } catch (error) {
